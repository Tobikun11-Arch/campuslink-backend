...
export async function recalcGamification() {
  ...
  for (const entry of aggregates) {
    ...
    for (const action of actions) {
      const value = contributionScores[action as keyof typeof contributionScores] ?? 0;
    ...
    const reputationScore = tradeCount + reports === 0 ? 0 : (tradeCount / (tradeCount + reports)) * 100;
    await userRepository.updateReputation(userId, Number(reputationScore.toFixed(2)));
  }

  const users = await userRepository.listAll();
  const sorted = [...users].sort((a, b) => (b.contributionScore ?? 0) - (a.contributionScore ?? 0));
